...
// to improve move ordering near the root. Tiered slot layout: pawn pushes
// [0,96), king back-rank [96,224), NORMAL rest [224,4320), PROMOTION
// [4320,4384), CASTLING [4384,4388).
...
sf_always_inline inline std::size_t low_ply_freq_index(Move m) {
    ...
    const std::uint32_t color     = std::uint32_t(tr < fr);
    const std::uint32_t is_cont   = std::uint32_t(((0x42u >> fr) & 1u) ^ 1u);
    const std::uint32_t is_double = std::uint32_t(((tr ^ fr) & 3u) == 2u);
    const std::uint32_t pawn_base = is_cont ? (color ? 64u : 32u) : (color ? 16u : 0u);
    const std::uint32_t pawn_off  = is_cont ? (fr - 2u) : is_double;
    const std::uint32_t pawn_slot = pawn_base + (ff << (1u + is_cont)) + pawn_off;

    const std::uint32_t fr_back  = std::uint32_t(fr == 0u || fr == 7u);
    ...
    std::size_t s = tier3;
    s             = king_hit ? king_slot : s;
    s             = pawn_hit ? pawn_slot : s;
    return s;
}
