...
constexpr int JOINT_CORR_BASE_SIZE     = 32768;  // has to be a power of 2
...
struct SharedHistories {
    SharedHistories(size_t threadCount) :
        ...
        jointCorrectionHistory2(std::min(threadCount, size_t(32))) {
        assert((threadCount & (threadCount - 1)) == 0 && threadCount != 0);
        sizeMinus1           = correctionHistory.get_size() - 1;
        pawnHistSizeMinus1   = pawnHistory.get_size() - 1;
        jointCorrSizeMinus1  = jointCorrectionHistory.get_size() - 1;
        jointCorrSize2Minus1 = jointCorrectionHistory2.get_size() - 1;
    }
    ...
    auto& joint_correction_entry2(size_t hash) {
        return jointCorrectionHistory2[hash & jointCorrSize2Minus1][0];
    }
    const auto& joint_correction_entry2(size_t hash) const {
        return jointCorrectionHistory2[hash & jointCorrSize2Minus1][0];
    }
    ...
    JointCorrectionHistory   jointCorrectionHistory2;

   private:
    size_t sizeMinus1, pawnHistSizeMinus1, jointCorrSizeMinus1, jointCorrSize2Minus1;
};
