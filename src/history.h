...
// history_slot maps a Move to a slot index in [0, HISTORY_SLOT_COUNT).
// The mapping is structural: pawn pushes / chebyshev=1 / knight L /
// 2-step ray / flat fallback / promotion / castling / en passant.
// All inputs map to a valid slot; corrupt or aliased TT moves cannot
// escape the table bounds.
constexpr int HISTORY_SLOT_COUNT = 5952;

extern const std::array<std::uint16_t, 0x4000> FREQ_SLOT_V8_NORMAL;
