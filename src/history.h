...
// LINEAR scales the bonus by (D - |val|) / D so writes shrink linearly as
// the entry saturates. Applied to correction history tables only.
...
struct StatsEntry {
   ...
   public:
    ...
    void operator<<(int bonus) {
        ...
        T   val          = *this;
        if constexpr (Dampening == CorrDampening::LINEAR)
        {
            const int headroom = D - std::abs(int(val));
            const int damp     = int(int64_t(clampedBonus) * headroom / int64_t(D));
...
using LinearStatsEntry = StatsEntry<T, D, Atomic, CorrDampening::LINEAR>;
