...
// plus 5-bit tag. Halves per-worker memory vs the 32-bit variant.
...
struct alignas(2) JointCorrSlot {
    ...
    uint16_t tag() const { return uint16_t(word >> VALUE_BITS); }

    static uint16_t pack(int v, uint16_t t) {
        v = std::clamp(v, VALUE_MIN, VALUE_MAX);
        return uint16_t((uint16_t(v) & VALUE_MASK) | uint16_t(t << VALUE_BITS));
    }
};
