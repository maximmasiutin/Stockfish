...
// Thread-local joint correction history: fixed 16384 int16 entries per
// worker (total memory still grows linearly with thread count).
...
// Thread-local joint correction table. Fixed 16384 int16 entries per
// worker (per-worker size is constant; total scales with thread count).
using JointCorrectionHistory = Stats<std::int16_t, CORRECTION_HISTORY_LIMIT, JOINT_CORR_BASE_SIZE>;
