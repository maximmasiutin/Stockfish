...
// ShiftStats is identical to Stats except StatsEntry::operator<< uses an
// arithmetic right shift by log2(D) instead of integer division. Reserved
// for histories whose D is a power of two and where the asymmetric rounding
// of signed shift on negative numerators is acceptable.
template<typename T, int D, std::size_t... Sizes>
...
// Pawn, minor, and non-pawn correction histories are atomic (shared between
// threads on a NUMA node) and opt in to the shift-based gravity update via
// the fourth StatsEntry template parameter.
template<typename T, int D>
struct CorrectionBundle {
    StatsEntry<T, D, true, true> pawn;
    StatsEntry<T, D, true, true> minor;
    StatsEntry<T, D, true, true> nonPawnWhite;
    StatsEntry<T, D, true, true> nonPawnBlack;
