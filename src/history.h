...
// Instrumentation: measure what a magnitude-gated eviction policy would do.
// Counters are global atomics (relaxed). On process exit a single dump file
// is written under scratchpad/contcorr-instrument/.
namespace ContCorrInstrument {

inline constexpr int N_BUCKETS   = 32;
inline constexpr int LOCK_THRESH = 263;
...
inline std::atomic<std::uint64_t> n_collision_locked{0};
...
inline std::string dump_path() {
    ...
    oss << "S:/q/stockfish-nnue/scratchpad/contcorr-instrument/dump-" << ns << ".txt";
    return oss.str();
}
...
inline void dump_to_file() {
    ...
    out << "# contcorr-hashed-i17-shift-pf-instrumentation dump\n";
    ...
    out << "collision_locked=" << n_collision_locked.load() << "\n";
    out << "hist_vold_bucket32=";
    for (int i = 0; i < N_BUCKETS; ++i)
        out << (i ? "," : "") << hist_vold[i].load();
    out << "\n";
    out << "hist_vfresh_bucket32=";
    for (int i = 0; i < N_BUCKETS; ++i)
...
struct alignas(2) HashedContCorrSlot {
    ...
    static inline sf_always_inline void update(HashedContCorrSlot& s, Tag t, int b) {
        ...
        ContCorrInstrument::n_writes.fetch_add(1, std::memory_order_relaxed);
        if (b != 0 && storedTag != int(t))
        {
            int v_fresh       = int(INIT) + b - ((int(INIT) * std::abs(b)) >> GRAVITY_SHIFT_BITS);
            v_fresh           = std::min(v_fresh, int(VALUE_MAX));
            const int abs_old = std::abs(v_old);
            const int abs_new = std::abs(v_fresh);
            ContCorrInstrument::n_writes_collision.fetch_add(1, std::memory_order_relaxed);
            if (abs_new <= abs_old)
                ContCorrInstrument::n_blocked_strict.fetch_add(1, std::memory_order_relaxed);
            if (abs_old > 2 * abs_new)
                ContCorrInstrument::n_blocked_2x.fetch_add(1, std::memory_order_relaxed);
            if (abs_old > 3 * abs_new)
                ContCorrInstrument::n_blocked_3x.fetch_add(1, std::memory_order_relaxed);
            if (abs_old > ContCorrInstrument::LOCK_THRESH)
                ContCorrInstrument::n_collision_locked.fetch_add(1, std::memory_order_relaxed);
            const int b_old = std::min(abs_old / 32, ContCorrInstrument::N_BUCKETS - 1);
            const int b_new = std::min(abs_new / 32, ContCorrInstrument::N_BUCKETS - 1);
            ContCorrInstrument::hist_vold[b_old].fetch_add(1, std::memory_order_relaxed);
            ContCorrInstrument::hist_vfresh[b_new].fetch_add(1, std::memory_order_relaxed);
        }

        int v = (storedTag == int(effectiveTag)) ? v_old : int(INIT);
