...
constexpr int MAGIC_HISTORY_SIZE       = UINT_16_HISTORY_SIZE;
...
// Frequency-aware magic index for the ButterflyHistory and LowPlyHistory
// addressing. Clusters frequently-accessed moves into low slot addresses to
// reduce cache pressure on the hot subset.
inline sf_always_inline constexpr std::uint16_t magic_index(std::uint16_t raw) {
    const std::uint16_t s1 = raw ^ static_cast<std::uint16_t>(raw << 11);
    const std::uint16_t r1 = ror16(s1, 10);
    const std::uint16_t s2 = raw ^ static_cast<std::uint16_t>(raw << 12);
    return 0x9327u ^ r1 ^ s2;
}
