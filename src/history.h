...
inline sf_always_inline constexpr std::uint16_t magic_index(std::uint16_t raw) {
    const std::uint16_t r1 = static_cast<std::uint16_t>(raw & 0xE004u);
    const std::uint16_t r2 = static_cast<std::uint16_t>(r1 ^ (r1 << 9));
    const std::uint16_t r3 = static_cast<std::uint16_t>(raw ^ (raw >> 3));
    return static_cast<std::uint16_t>(r2 ^ r3);
}
