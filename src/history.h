...
// toward saturation). Opposite-sign bonuses pass through undamped so they
// can restore the entry toward zero at full strength. Applied to
// correction history tables only.
...
struct CorrectionBundle {
    QuadraticStatsEntry<T, D, true> pawn;
    QuadraticStatsEntry<T, D, true> minor;
    QuadraticStatsEntry<T, D, true> nonPawnWhite;
    QuadraticStatsEntry<T, D, true> nonPawnBlack;
...
struct SharedHistories {
    using AtomicPieceToCorrHist =
      QuadraticAtomicStats<std::int16_t, CORRECTION_HISTORY_LIMIT, PIECE_NB, SQUARE_NB>;
