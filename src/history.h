...
inline sf_always_inline constexpr std::uint16_t magic_index(std::uint16_t raw) {
    const std::uint16_t s1 =
      static_cast<std::uint16_t>(raw ^ static_cast<std::uint16_t>(raw << 11));
    const std::uint16_t r1 = ror16(s1, 10);
    const std::uint16_t s2 =
      static_cast<std::uint16_t>(raw ^ static_cast<std::uint16_t>(raw << 12));
    return static_cast<std::uint16_t>(0xB3ACu ^ r1 ^ s2);
}
