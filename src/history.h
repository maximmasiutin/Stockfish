...
constexpr int BIJECT_HISTORY_SIZE      = 8192;  // compact butterfly + lowply slot count
...
static_assert((BIJECT_HISTORY_SIZE & (BIJECT_HISTORY_SIZE - 1)) == 0,
              "BIJECT_HISTORY_SIZE has to be a power of 2");
...
// Compact bijection index for ButterflyHistory and LowPlyHistory. Maps raw moves
// into a 13-bit slot range [0, 8191]. NORMAL moves keep their (from<<6)|to
// low12 layout for cache-line-pattern continuity with master, and non-NORMAL
// moves are placed in the upper half via bit 12. The XOR mix of hi nibble
// into bits 8-11 keeps the per-type encoding distinct across PROMOTION and
// CASTLING active raws.
...
constexpr std::uint32_t biject_index_max_over_uint16() {
    ...
    {
        std::uint16_t y = biject_index(static_cast<std::uint16_t>(r));
        if (y > m)
            m = y;
    }
    return m;
...
class alignas(64) BijectIndexedArray: public MultiArray<T, Size> {
   public:
    ...
    inline sf_always_inline constexpr T& operator[](Move m) noexcept {
        const std::uint16_t idx = biject_index(m.raw());
        assert(idx < Size);
...
class BijectIndexedHistory {
    std::array<BijectIndexedArray<T, Size>, Outer> data_;
...
using ButterflyHistory =
  BijectIndexedHistory<StatsEntry<std::int16_t, 7183>, COLOR_NB, BIJECT_HISTORY_SIZE>;

// LowPlyHistory is addressed by ply and move's from and to squares, used
// to improve move ordering near the root
using LowPlyHistory =
  BijectIndexedHistory<StatsEntry<std::int16_t, 7183>, LOW_PLY_HISTORY_SIZE, BIJECT_HISTORY_SIZE>;
