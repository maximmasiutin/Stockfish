...
struct StatsEntry {
   ...
   public:
    ...
    void operator<<(int bonus) {
        ...
        // Quadratic headroom dampening for correction history
        if constexpr (D == CORRECTION_HISTORY_LIMIT)
        {
            const int headroom = D - std::abs(val);
            clampedBonus       = std::clamp(
              int(int64_t(clampedBonus) * headroom * headroom / (int64_t(D) * D)), -D, D);
        }
...
// Atomic version of PieceTo correction history for NUMA-shared access
