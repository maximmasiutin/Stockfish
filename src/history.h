...
inline sf_always_inline constexpr std::uint32_t magic_index_mh(std::uint32_t raw) {
    return (raw ^ (raw >> 5)) ^ 0x4FC5u;
}

inline sf_always_inline constexpr std::uint32_t magic_index_lp(std::uint32_t raw) { return raw; }

template<typename T, std::size_t Size, std::uint32_t (*FN)(std::uint32_t)>
class alignas(64) MoveIndexedArray: public MultiArray<T, Size> {
   public:
    ...
    inline sf_always_inline constexpr T& operator[](Move m) noexcept {
        const std::uint32_t idx = FN(m.raw());
        assert(idx < Size);
...
template<typename T, std::size_t Outer, std::size_t Size, std::uint32_t (*FN)(std::uint32_t)>
