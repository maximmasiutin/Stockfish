...
// LINEAR_SAME_SIGN scales the bonus by (D - |val|) / D only when the bonus
// has the same sign as val (and so pushes the entry further toward
// saturation). Opposite-sign bonuses pass through undamped so they can
// restore the entry toward zero at full strength. Applied to correction
// history tables only.
...
struct StatsEntry {
   ...
   public:
    ...
    void operator<<(int bonus) {
        ...
        T   val          = *this;
        if constexpr (Dampening == CorrDampening::LINEAR_SAME_SIGN)
        {
            ...
            // std::min(..., D) communicates the structural invariant |val| <= D
            // to the optimizer so the inner clamp is provably redundant.
            const int absVal       = std::min(std::abs(int(val)), D);
            const int diffSignMask = (int(val) ^ clampedBonus) >> 31;
            const int absValGated  = absVal & ~diffSignMask;
            const int headroom     = D - absValGated;
            clampedBonus           = int(int64_t(clampedBonus) * headroom / int64_t(D));
        }
...
using LinearStatsEntry = StatsEntry<T, D, Atomic, CorrDampening::LINEAR_SAME_SIGN>;

template<typename T, int D, std::size_t... Sizes>
using LinearStats = MultiArray<LinearStatsEntry<T, D, false>, Sizes...>;

template<typename T, int D, std::size_t... Sizes>
using LinearAtomicStats = MultiArray<LinearStatsEntry<T, D, true>, Sizes...>;
...
struct CorrectionBundle {
    LinearStatsEntry<T, D, true> pawn;
    LinearStatsEntry<T, D, true> minor;
    LinearStatsEntry<T, D, true> nonPawnWhite;
    LinearStatsEntry<T, D, true> nonPawnBlack;
...
struct CorrHistTypedef {
    using type =
      DynStats<LinearStats<std::int16_t, CORRECTION_HISTORY_LIMIT, COLOR_NB>, CORRHIST_BASE_SIZE>;
};

template<>
struct CorrHistTypedef<PieceTo> {
    using type = LinearStats<std::int16_t, CORRECTION_HISTORY_LIMIT, PIECE_NB, SQUARE_NB>;
};
...
struct CorrHistTypedef<NonPawn> {
    using type = DynStats<LinearStats<std::int16_t, CORRECTION_HISTORY_LIMIT, COLOR_NB, COLOR_NB>,
                          CORRHIST_BASE_SIZE>;
};
...
struct SharedHistories {
    using AtomicPieceToCorrHist =
      LinearAtomicStats<std::int16_t, CORRECTION_HISTORY_LIMIT, PIECE_NB, SQUARE_NB>;
    ...
    void clear_contcorr_range(size_t threadIdx, size_t numaTotal) {
        ...
        constexpr int    correctionHistoryFill = 6;
