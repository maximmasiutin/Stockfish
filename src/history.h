...
inline sf_always_inline constexpr std::uint16_t magic_index_mh(std::uint16_t raw) {
    return std::uint16_t((raw ^ 0x0290u ^ rol16(raw & 0x003Fu, 6)) - 0x49u);
}

// Frequency-aware magic index for LowPlyHistory addressing; coefficients tuned
// for the lowPlyHistory access set.
inline sf_always_inline constexpr std::uint16_t magic_index_lp(std::uint16_t raw) {
    return std::uint16_t((raw ^ 0x3672u) ^ (rol16(raw, 6) & 0x0FE3u));
}
...
class MagicIndexedHistory {
    std::array<MagicIndexedArray<T, Size, FN>, Outer> data_;
...
using ButterflyHistory = MagicIndexedHistory<StatsEntry<std::int16_t, 7183>,
                                             COLOR_NB,
...
using LowPlyHistory = MagicIndexedHistory<StatsEntry<std::int16_t, 7183>,
