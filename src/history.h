...
// Maps a quiet Move's raw() to a frequency-tiered LowPly slot. The tier
// formula assumes QUIETS-phase movegen invariants (no EN_PASSANT; promotions
// are KNIGHT/BISHOP/ROOK only, never QUEEN); calling with a non-quiet move
// will assert in debug. sf_noinline keeps the tier dispatch out of the
// call-site BTB.
