...
namespace Stockfish::Simd {
    ...
    [[maybe_unused]] static void m256_split_m128x2(__m256i r, __m128i &hi, __m128i &lo) {
        ...
        const __m128 l = _mm256_castps256_ps128(rps); // convert __m256 to __m128 with data loss of highest 128 bits, leaving only lowest 128 bits
    ...
    [[maybe_unused]] static void m512_split_m128x4(__m512i r, __m128i &x4, __m128i &x3, __m128i &x2, __m128i &x1) {
        const __m512 rps = _mm512_castsi512_ps(r); // convert __m512i to __m512
        const __m256 h = _mm512_extractf32x8_ps(rps, 1); // extract highest 256 bits of __m512 to __m256
        const __m256 l = _mm512_castps512_ps256(rps); // convert __m512 to __m256 with data loss of higest 256 bits, leaving only lowest 256 bits
        m256_split_m128x2(_mm256_castps_si256(h), x4, x3); // split one __m256i h into two __m128i x4 and x3 
