...
struct AccumulatorUpdateContext {
    ...
    void apply(const typename FeatureSet::IndexList& added,
        ...
        for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
        {
            ...
            // AVX-512 ICL: 2-way unroll with pre-combine to break acc dependency chain.
            // Instead of two sequential ops on acc per pair, pre-combine widened columns
            // independently, then apply once. This shortens the acc dependency chain from
            // 2 to 1 per pair, improving ILP.
            {
                int i = 0;
                for (; i + 1 < removed.ssize(); i += 2)
                {
                    ...
                    for (IndexType k = 0; k < Tiling::NumRegs; ++k)
                    {
                        const vec_t combined =
                          vec_add_16(vec_convert_8_16(col0[k]), vec_convert_8_16(col1[k]));
                        acc[k] = vec_sub_16(acc[k], combined);
                    }
                }
            ...
            }
            {
                ...
                for (; i + 1 < added.ssize(); i += 2)
                {
                    ...
                    for (IndexType k = 0; k < Tiling::NumRegs; ++k)
                    {
                        const vec_t combined =
                          vec_add_16(vec_convert_8_16(col0[k]), vec_convert_8_16(col1[k]));
                        acc[k] = vec_add_16(acc[k], combined);
                    }
                }
                for (; i < added.ssize(); ++i)
                {
