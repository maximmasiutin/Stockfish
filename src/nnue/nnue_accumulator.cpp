...
Bitboard get_changed_pieces(const std::array<Piece, SQUARE_NB>& oldPieces,
    ...
    // Single 512-bit load and compare - no loop needed
    static_assert(sizeof(Piece) == 1);
    const __m512i old_v = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(&oldPieces[0]));
    const __m512i new_v = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(&newPieces[0]));
    const __mmask64 sameMask = _mm512_cmpeq_epi8_mask(old_v, new_v);
    return ~static_cast<Bitboard>(sameMask);
#elif defined(USE_AVX2)
    // All loads issued in parallel - no loop-carried dependency
    static_assert(sizeof(Piece) == 1);
