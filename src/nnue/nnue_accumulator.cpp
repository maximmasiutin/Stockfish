...
struct AccumulatorUpdateContext {
    ...
    // Entry-first apply: process all tiles per entry for better prefetch timing
    void apply(const typename FeatureSet::IndexList& added,
        ...
        // Copy accumulator first (gives ~200 cycles for first prefetch)
        ...
        {
            // Prefetch next entry (T0) - 16 tiles (~800 cycles) lead time
            if (i + 1 < total)
