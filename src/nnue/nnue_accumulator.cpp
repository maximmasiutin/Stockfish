...
struct AccumulatorUpdateContext {
    ...
    void apply(const typename FeatureSet::IndexList& added,
        ...
        for (int i = removed.ssize() - 1; i >= 0; --i)
        {
            const auto* nextCol = (i > 0) ? baseWeights + Dimensions * removed[i - 1]
                                : (added.ssize() > 0)
                                  ? baseWeights + Dimensions * added[added.ssize() - 1]
                                  : nullptr;
            if (nextCol != nullptr)
            {
                prefetch<PrefetchRw::READ, PrefetchLoc::LOW>(nextCol);
                prefetch<PrefetchRw::READ, PrefetchLoc::LOW>(nextCol + Tiling::TileHeight);
            }
            ...
            for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
            {
                ...
                for (IndexType k = 0; k < Tiling::NumRegs; k += 2)
                {
                    ...
                    acc[k + 1] = vec_load(&tile[k + 1]);
                    acc[k]     = vsubw_s8(acc[k], vget_low_s8(col[k / 2]));
                    acc[k + 1] = vsubw_high_s8(acc[k + 1], col[k / 2]);
                    vec_store(&tile[k], acc[k]);
                    vec_store(&tile[k + 1], acc[k + 1]);
                }
    #else
                for (IndexType k = 0; k < Tiling::NumRegs; k += 2)
                {
                    vec_t a0 = vec_load(&tile[k]);
                    vec_t a1 = vec_load(&tile[k + 1]);
                    a0       = vec_sub_16(a0, vec_convert_8_16(col[k]));
                    a1       = vec_sub_16(a1, vec_convert_8_16(col[k + 1]));
                    vec_store(&tile[k], a0);
                    vec_store(&tile[k + 1], a1);
                }
    #endif
            }
        }

        for (int i = added.ssize() - 1; i >= 0; --i)
        {
            const auto* nextCol = (i > 0) ? baseWeights + Dimensions * added[i - 1] : nullptr;
            if (nextCol != nullptr)
            {
                prefetch<PrefetchRw::READ, PrefetchLoc::LOW>(nextCol);
                prefetch<PrefetchRw::READ, PrefetchLoc::LOW>(nextCol + Tiling::TileHeight);
            ...
            for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
            {
                ...
                for (IndexType k = 0; k < Tiling::NumRegs; k += 2)
                {
                    vec_t a0 = vec_load(&tile[k]);
                    vec_t a1 = vec_load(&tile[k + 1]);
                    a0       = vec_add_16(a0, vec_convert_8_16(col[k]));
                    a1       = vec_add_16(a1, vec_convert_8_16(col[k + 1]));
                    vec_store(&tile[k], a0);
                    vec_store(&tile[k + 1], a1);
                }
    #endif
