...
struct AccumulatorUpdateContext {
    ...
    void apply(const typename FeatureSet::IndexList& added,
        ...
        for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
        {
            ...
            // Process removed columns two at a time for memory-level parallelism
            int i = 0;
            ...
            // Process added columns two at a time for memory-level parallelism
            i = 0;
        ...
        for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j)
        {
            ...
            for (; pi + 1 < removed.ssize(); pi += 2)
            {
                ...
                const size_t off1 = PSQTBuckets * size_t(removed[pi + 1]) + j * Tiling::PsqtTileHeight;
                auto* pCol0 = reinterpret_cast<const psqt_vec_t*>(
                  &featureTransformer.threatPsqtWeights[off0]);
                auto* pCol1 = reinterpret_cast<const psqt_vec_t*>(
                  &featureTransformer.threatPsqtWeights[off1]);
            ...
            {
                const size_t offset = PSQTBuckets * size_t(removed[pi]) + j * Tiling::PsqtTileHeight;
            ...
            for (; pi + 1 < added.ssize(); pi += 2)
            {
                ...
                const size_t off1 = PSQTBuckets * size_t(added[pi + 1]) + j * Tiling::PsqtTileHeight;
                auto* pCol0 = reinterpret_cast<const psqt_vec_t*>(
                  &featureTransformer.threatPsqtWeights[off0]);
                auto* pCol1 = reinterpret_cast<const psqt_vec_t*>(
                  &featureTransformer.threatPsqtWeights[off1]);
