...
struct AccumulatorUpdateContext {
    ...
    void apply(const typename FeatureSet::IndexList& added,
        ...
        for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j)
        {
            ...
            // AVX-512 ICL: 2-way unroll + prefetch, keeps loops separate to avoid doubled bandwidth
            {
                int i = 0;
                for (; i + 1 < removed.ssize(); i += 2)
                {
                    ...
                    // Prefetch next iteration or first added
                ...
                // Handle remaining removed feature
            ...
            // AVX-512 ICL: 2-way unroll with prefetch for added features
            {
                int i = 0;
                for (; i + 1 < added.ssize(); i += 2)
                {
                    ...
                    if (i + 2 < added.ssize())
                        _mm_prefetch(
                          reinterpret_cast<const char*>(&threatWeights[Dimensions * added[i + 2]]),
                          _MM_HINT_T0);
                ...
                // Handle remaining added feature
                for (; i < added.ssize(); ++i)
                {
