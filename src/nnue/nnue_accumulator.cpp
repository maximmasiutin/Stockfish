...
struct AccumulatorUpdateContext {
    ...
    // Template-unrolled accumulator update: N is compile-time constant.
    // GCC unrolls N<=3 fully; N>=4 becomes a runtime loop.
    ...
    static void dispatch_acc(vec_t*                                acc,
        ...
        int i = 0;
        for (; i + 3 <= count; i += 3)
            apply_acc_chunk<3, IsAdd, Tiling>(acc, list, tw, i);
        switch (count - i)
        {
        case 2 :
            apply_acc_chunk<2, IsAdd, Tiling>(acc, list, tw, i);
            break;
        case 1 :
            apply_acc_chunk<1, IsAdd, Tiling>(acc, list, tw, i);
            break;
        case 0 :
            break;
        }
    ...
    static void dispatch_psqt(psqt_vec_t*                           psqt,
        ...
        int i = 0;
        for (; i + 3 <= count; i += 3)
            apply_psqt_chunk<3, IsAdd, Tiling>(psqt, list, tpw, j, i);
        switch (count - i)
        {
        case 2 :
            apply_psqt_chunk<2, IsAdd, Tiling>(psqt, list, tpw, j, i);
            break;
        case 1 :
            apply_psqt_chunk<1, IsAdd, Tiling>(psqt, list, tpw, j, i);
            break;
        case 0 :
            break;
        }
