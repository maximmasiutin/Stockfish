...
// present at the given bitboard. Branchless version - always writes, empty mask writes nothing.
...
void write_multiple_dirties(const Position& p,
    ...
    // Early exit for empty mask - branchless alternative below if needed
    if (dt_count == 0)
        return;
