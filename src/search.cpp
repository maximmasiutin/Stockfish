...
void update_correction_history(const Position& pos,
    ...
    shared.pawn_correction_entry(pos).at(us).pawn << bonus / 8;
    shared.minor_piece_correction_entry(pos).at(us).minor << bonus * 153 / 1024;
    shared.nonpawn_correction_entry<WHITE>(pos).at(us).nonPawnWhite << bonus * nonPawnWeight / 1024;
    shared.nonpawn_correction_entry<BLACK>(pos).at(us).nonPawnBlack << bonus * nonPawnWeight / 1024;

    if (m.is_ok())
    {
        const Square to = m.to_sq();
        const Piece  pc = pos.piece_on(to);
        (*(ss - 2)->continuationCorrectionHistory)[pc][to] << bonus * 126 / 1024;
        (*(ss - 4)->continuationCorrectionHistory)[pc][to] << bonus * 63 / 1024;
    }
}
...
moves_loop:  // When in check, search starts here
    ...
    {
        constexpr int bonusClamp = 38912;
        auto bonus = std::clamp(int(bestValue - ss->staticEval) * depth * (bestMove ? 12 : 17),
                                -bonusClamp, bonusClamp);
