...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int jcv =
      (ss - 2)->currentMove.is_ok() && (ss - 3)->currentMove.is_ok() && ss->ply >= 3
        ? int(shared.joint_correction_entry(size_t((ss - 2)->contcorrIndex) * CONTCORR_HASH_MULT
                                            + (ss - 3)->contcorrIndex))
        : 0;
...
void update_correction_history(const Position& pos,
    ...
    {
        shared.joint_correction_entry(size_t((ss - 2)->contcorrIndex) * CONTCORR_HASH_MULT
                                      + (ss - 3)->contcorrIndex)
          << bonus;
    }
}
