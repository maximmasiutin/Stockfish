...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    ...
    if (((ss - 3)->currentMove).is_ok())
        (*(ss - 3)->continuationHistory)[pc][to] << (bonus * 312 / 1024) * notInCheck;
    if (((ss - 4)->currentMove).is_ok())
        (*(ss - 4)->continuationHistory)[pc][to] << (bonus * 582 / 1024) * notInCheck;
    if (((ss - 5)->currentMove).is_ok())
        (*(ss - 5)->continuationHistory)[pc][to] << (bonus * 149 / 1024) * notInCheck;
    if (((ss - 6)->currentMove).is_ok())
        (*(ss - 6)->continuationHistory)[pc][to] << (bonus * 474 / 1024) * notInCheck;
}
