...
Search::Worker::Worker(SharedState&                    sharedState,
    ...
    // Thread-local correction history; pawn history stays shared
...
void Search::Worker::do_move(
  Position& pos, const Move move, StateInfo& st, const bool givesCheck, Stack* const ss) {
    ...
    pos.do_move(move, st, givesCheck, dirtyPiece, dirtyThreats, &tt, &globalHistory);
...
void Search::Worker::clear() {
    ...
    // Clear thread-local correction history (sole owner, full range)
...
// Sync correction entries from global to local for current position.
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // Sync correction from global to local every 2048 nodes
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else
            {
                int history = (*contHist[0])[movedPiece][move.to_sq()]
                            ...
                            + globalHistory.pawn_entry(pos)[movedPiece][move.to_sq()];
