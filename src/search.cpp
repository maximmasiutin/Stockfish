...
void update_all_stats(const Position& pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        int      clampedDepth = std::clamp(int(depth) - 1, 0, 21);
        unsigned lowConfMul   = 512 + 492 * unsigned(clampedDepth) / 21;
        size_t   i            = 0;
        for (Move move : quietsSearched)
        {
            ...
            unsigned mult         = quietsLowConf[i++] ? lowConfMul : 1024;
            int      contribution = -int(actualMalus * mult / 1024);
