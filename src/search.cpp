...
// Horner: idxA*P + idxB. idxA is (ss-1), idxB is (ss-2) for T1 or (ss-4) for T2.
// Full enumeration over odd primes in [5, 3527) at N=16384 for the 769-entry
// Piece-based domain (12 pieces * 64 squares + NO_PIECE sentinel) selects
// P=383 as top-1 for 2-ply uniformity (std=0.594, chi2/mean=160.3), more
// than a 3.7x improvement over the next best candidate. Don't SPSA-tune:
// hash multipliers have no smooth optimum.
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int   cntcv  = (ss - 1)->currentMove.is_ok()
                         ? int(w.jointCorrectionHistory[ss->contcorrHash & JOINT_CORR_MASK])
                          + int(w.jointCorrectionHistory[ss->contcorrHash2 & JOINT_CORR_MASK])
                         : JOINT_CORR_NOK;
...
void update_correction_history(const Position& pos,
    ...
    {
        workerThread.jointCorrectionHistory[ss->contcorrHash & JOINT_CORR_MASK] << bonus;
        workerThread.jointCorrectionHistory[ss->contcorrHash2 & JOINT_CORR_MASK] << bonus / 2;
    }
}
...
void Search::Worker::clear() {
    ...
    jointCorrectionHistory.fill(JOINT_CORR_INIT);
