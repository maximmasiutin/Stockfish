...
bool Search::Worker::iterative_deepening() {
    ...
    for (Color c : {WHITE, BLACK})
        for (int i = 0; i < FREQ_SLOT_COUNT; i++)
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        mainHistory[~us][((ss - 1)->currentMove).slot()] << evalDiff * 10;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else if (!ss->followPV || !PvNode)
            {
                ...
                history += 71 * mainHistory[us][move.slot()] / 32;
        ...
        if (capture)
            ...
            ss->statScore = 2 * mainHistory[us][move.slot()]
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        mainHistory[~us][((ss - 1)->currentMove).slot()] << scaledBonus * 235 / 32768;
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    Color us = pos.side_to_move();
    workerThread.mainHistory[us][move.slot()] << bonus;  // Untuned to prevent duplicate effort

    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.slot()] << bonus * 682 / 1024;
