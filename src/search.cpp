...
// Pair NO_PIECE only with SQ_A1: (NO_PIECE, other sq) aliases a real
// (piece, sq) bucket.
unsigned contcorr_index(Piece pc, Square sq) { return unsigned(pc) * 64 + unsigned(sq); }

// Horner: ((idxA*P1 + idxB)*P2 + idxC). idxA becomes child-(ss-1), idxB
// becomes child-(ss-2), idxC becomes child-(ss-4); larger primes
// multiply nearer ancestors.
uint32_t contcorr_hash_3ply(unsigned idxA, unsigned idxB, unsigned idxC) {
    // FFT enumeration over odd primes in [257, 12280] against the 769-
    // element Piece-based index domain for N=32768: inner P=3527 (top-1
    // at 2-ply), outer P=2111 (top-1 at 3-ply conditional on inner=3527,
    // std=1.073, chi2/mean=1.150).
    // Don't SPSA-tune: hash multipliers have no smooth optimum.
    constexpr uint32_t primeHashMult2Ply = 3527;
    constexpr uint32_t primeHashMult3Ply = 2111;
    return (idxA * primeHashMult2Ply + idxB) * primeHashMult3Ply + idxC;
}
...
void Search::Worker::do_move(
  Position& pos, const Move move, StateInfo& st, const bool givesCheck, Stack* const ss) {
    ...
    if (ss != nullptr)
    {
        ...
        // Seed child hash: at the read site ss becomes (ss-1), our
        // (ss-1) becomes (ss-2), and our (ss-3) becomes (ss-4). The
        // one-ply write-site shift is intentional.
        (ss + 1)->contcorrHash =
          contcorr_hash_3ply(ss->contcorrIndex, (ss - 1)->contcorrIndex, (ss - 3)->contcorrIndex);
    }
}
