...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    ...
    // Compute all 6 addresses unconditionally
    ...
    // Branchless prefetch all 6 entries
    prefetch(addr1);
    prefetch(addr2);
    prefetch(addr3);
    prefetch(addr4);
    ...
    if (((ss - 1)->currentMove).is_ok())
        *addr1 << (bonus * 1133 / 1024) + 88;
    if (((ss - 2)->currentMove).is_ok())
        *addr2 << (bonus * 683 / 1024);
    if (notInCheck && ((ss - 3)->currentMove).is_ok())
        *addr3 << (bonus * 312 / 1024);
    if (notInCheck && ((ss - 4)->currentMove).is_ok())
        *addr4 << (bonus * 582 / 1024);
    if (notInCheck && ((ss - 5)->currentMove).is_ok())
        *addr5 << (bonus * 149 / 1024);
    if (notInCheck && ((ss - 6)->currentMove).is_ok())
        *addr6 << (bonus * 474 / 1024);
}
