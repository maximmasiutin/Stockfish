...
// child-(ss-2); larger prime multiplies the nearer ancestor.
uint32_t contcorr_hash_2ply(unsigned idxA, unsigned idxB) {
    // FFT enumeration over odd primes against the 769-element Piece-
    // based domain selects P1=3527 as top-1 for 2-ply uniformity at
    // N=32768 (std=0.330, chi2/mean=111.6).
...
void Search::Worker::do_move(
  Position& pos, const Move move, StateInfo& st, const bool givesCheck, Stack* const ss) {
    ...
    if (ss != nullptr)
    {
        ...
        // Seed the child ply's joint-correction hash from the just-played
        // move (ss->contcorrIndex, which becomes (ss-1) at the child read
        // site) and the prior ancestor index ((ss-1)->contcorrIndex,
        // which becomes (ss-2)). The one-ply write-site shift is
        // intentional.
