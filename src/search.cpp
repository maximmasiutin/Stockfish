...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const auto& pmEntry = shared.pawnminor_correction_entry(pos).at(us);
    const int   pcv     = pmEntry.pawn;
    const int   micv    = pmEntry.minor;
...
void update_correction_history(const Position& pos,
    ...
    auto& pmEntry = shared.pawnminor_correction_entry(pos).at(us);
    pmEntry.pawn << bonus;
    pmEntry.minor << bonus * 155 / 128;
