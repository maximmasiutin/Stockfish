...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int   cntcv =
      m.is_ok() ? contcorr_val(*(ss - 2)->continuationCorrectionHistory, pos.piece_on(m.to_sq()), m.to_sq())
                    + contcorr_val(*(ss - 4)->continuationCorrectionHistory, pos.piece_on(m.to_sq()), m.to_sq())
                  : CONTCORR_FALLBACK;
...
void update_correction_history(const Position& pos,
    ...
    contcorr_update((*(ss - 2)->continuationCorrectionHistory)[pc][to], bonus2);
    contcorr_update((*(ss - 4)->continuationCorrectionHistory)[pc][to], bonus4);
}
...
void Search::Worker::clear() {
    ...
    sharedHistory.clear_contcorr_range(CONTCORR_FILL, numaThreadIdx, numaTotal);
