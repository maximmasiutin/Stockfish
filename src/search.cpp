...
// Scale factor for correction_value() threshold comparisons.
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && !is_loss(beta))
    {
        ...
        // Null move dynamic reduction based on depth and positive correction magnitude.
        Depth R = 7 + depth / 3;

        // Graduated: +1 for moderate underestimation, +2 for extreme.
        R += (correctionValue > 192 * CORRECTION_CENTIPAWN_SCALE)
           + (correctionValue > 224 * CORRECTION_CENTIPAWN_SCALE);
