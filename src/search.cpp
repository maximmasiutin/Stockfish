...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int jcv = ss->ply >= 4 && (ss - 2)->currentMove.is_ok() && (ss - 3)->currentMove.is_ok()
                     && (ss - 4)->currentMove.is_ok()
                    ? int(shared.joint_correction_entry(size_t(
                        ((ss - 2)->contcorrIndex * CONTCORR_HASH_MULT + (ss - 3)->contcorrIndex)
                          * CONTCORR_HASH_MULT
                        + (ss - 4)->contcorrIndex)))
                    : 0;
...
void update_correction_history(const Position& pos,
        ...
        && (ss - 4)->currentMove.is_ok())
    {
        shared.joint_correction_entry(
          size_t(((ss - 2)->contcorrIndex * CONTCORR_HASH_MULT + (ss - 3)->contcorrIndex)
                   * CONTCORR_HASH_MULT
                 + (ss - 4)->contcorrIndex))
          << bonus;
    }
