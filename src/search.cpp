...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    if ((ss - 1)->currentMove.is_ok())
    {
        ...
        cntcv = read(wA, uint16_t(ss->contcorrTagA)) + read(wB, uint16_t(ss->contcorrTagB));
    }
    else
...
void update_correction_history(const Position& pos,
    ...
    if ((ss - 1)->currentMove.is_ok())
    {
        ...
        auto update = [](JointCorrSlot& s, uint16_t tag, int b) {
        ...
        update(workerThread.jointCorrectionHistory[ss->contcorrIdxA], uint16_t(ss->contcorrTagA),
               bonus);
        update(workerThread.jointCorrectionHistory[ss->contcorrIdxB], uint16_t(ss->contcorrTagB),
               bonus / 2);
    }
}
