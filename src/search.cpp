...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    ...
    (*(ss - 1)->continuationHistory)[pc][to]
      << (((ss - 1)->currentMove).is_ok() ? bonus * 1133 / 1024 + 88 : 0);
    (*(ss - 2)->continuationHistory)[pc][to]
      << (((ss - 2)->currentMove).is_ok() ? bonus * 683 / 1024 : 0);
    (*(ss - 3)->continuationHistory)[pc][to]
      << (notInCheck && ((ss - 3)->currentMove).is_ok() ? bonus * 312 / 1024 : 0);
    (*(ss - 4)->continuationHistory)[pc][to]
      << (notInCheck && ((ss - 4)->currentMove).is_ok() ? bonus * 582 / 1024 : 0);
    (*(ss - 5)->continuationHistory)[pc][to]
      << (notInCheck && ((ss - 5)->currentMove).is_ok() ? bonus * 149 / 1024 : 0);
    (*(ss - 6)->continuationHistory)[pc][to]
      << (notInCheck && ((ss - 6)->currentMove).is_ok() ? bonus * 474 / 1024 : 0);
}
