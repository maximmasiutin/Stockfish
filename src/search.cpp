...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    if (m.is_ok())
    {
        ...
        cntcv                      = HashedContCorrSlot::read(table[idxA].word, tagA)
              + HashedContCorrSlot::read(table[idxB].word, tagB);
    }
    else
...
void update_correction_history(const Position& pos,
    ...
    if (m.is_ok())
    {
        ...
        const auto   tagA          = contcorr_tag_from(hashA);
        const auto   tagB          = contcorr_tag_from(hashB);
        HashedContCorrSlot::update(table[idxA], tagA, bonus * 126 / 128);
        HashedContCorrSlot::update(table[idxB], tagB, bonus * 63 / 128);
    }
}
