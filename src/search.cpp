...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const HashedContCorrSlot* slotA = nullptr;
    const HashedContCorrSlot* slotB = nullptr;
    HashedContCorrSlot::Tag   tagA  = 0;
    HashedContCorrSlot::Tag   tagB  = 0;
    if (m.is_ok())
    {
        ...
        slotA                      = &table[contcorr_idx_from(hashA)];
        slotB                      = &table[contcorr_idx_from(hashB)];
        tagA                       = contcorr_tag_from(hashA);
        tagB                       = contcorr_tag_from(hashB);
        prefetch(slotA);
        prefetch(slotB);
    }
    ...
    if (m.is_ok())
    {
        const HashedContCorrReadAccess accA{slotA, tagA};
        const HashedContCorrReadAccess accB{slotB, tagB};
