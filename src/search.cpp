...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && !is_loss(beta))
    {
        ...
        // Null move dynamic reduction based on depth and correction value
        Depth R = 7 + depth / 3;

        // Correction for NNUE estimation bias at centipawn thresholds
        R += (correctionValue > 192 * CORRECTION_CENTIPAWN_SCALE)
           + (correctionValue > 384 * CORRECTION_CENTIPAWN_SCALE)
           - (correctionValue < -192 * CORRECTION_CENTIPAWN_SCALE)
           - (correctionValue < -384 * CORRECTION_CENTIPAWN_SCALE);
