...
bool Search::Worker::iterative_deepening() {
    ...
    for (int i = 7; i > 0; --i)
    {
        (ss - i)->continuationHistory =
          &continuationHistory[0][0][cont_hist_piece_index(NO_PIECE)][0];  // Use as a sentinel
...
void Search::Worker::do_null_move(Position& pos, StateInfo& st, Stack* const ss) {
    ...
    ss->continuationHistory = &continuationHistory[0][0][cont_hist_piece_index(NO_PIECE)][0];
