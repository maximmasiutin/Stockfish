...
void Search::Worker::iterative_deepening() {
    ...
    for (int i = 7; i > 0; --i)
    {
        (ss - i)->continuationHistory           = &sharedHistory.conthist_sentinel();
...
void Search::Worker::do_move(
  Position& pos, const Move move, StateInfo& st, const bool givesCheck, Stack* const ss) {
    ...
    {
        ss->currentMove         = move;
        ss->continuationHistory = &sharedHistory.conthist_entry(
          pos.pawn_key(), ss->inCheck, capture, dirtyPiece.pc, move.to_sq());
...
void Search::Worker::do_null_move(Position& pos, StateInfo& st, Stack* const ss) {
    ...
    ss->continuationHistory           = &sharedHistory.conthist_sentinel();
...
void Search::Worker::clear() {
    ...
    sharedHistory.contHistTable.clear_range(-541 / CONTHIST_SCALE, numaThreadIdx, numaTotal);
    sharedHistory.conthist_sentinel().fill(-541 / CONTHIST_SCALE);
...
moves_loop:  // When in check, search starts here
    ...
    const CompactPieceToHistory* contHist[] = {
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            {
                int history = conthist_val(*contHist[0], movedPiece, move.to_sq())
                            + conthist_val(*contHist[1], movedPiece, move.to_sq())
        ...
        if (capture)
            ...
            ss->statScore = 2 * mainHistory[us][move.raw()]
                          + conthist_val(*contHist[0], movedPiece, move.to_sq())
                          + conthist_val(*contHist[1], movedPiece, move.to_sq());
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    const CompactPieceToHistory* contHist[] = {(ss - 1)->continuationHistory};
    ...
    for (const auto [i, weight] : conthist_bonuses)
    {
        ...
        if (((ss - i)->currentMove).is_ok())
            (*(ss - i)->continuationHistory)[pc][to]
              << ((bonus * weight / 1024) + 82 * (i < 2)) / CONTHIST_SCALE;
    }
}
