...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && !is_loss(beta))
    {
        ...
        // Prune less when negative correction exceeds centipawn thresholds
        bool     step1 = correctionValue < -192 * CORRECTION_CENTIPAWN_SCALE;
        bool     step2 = correctionValue < -256 * CORRECTION_CENTIPAWN_SCALE;
        bool     step3 = correctionValue < -288 * CORRECTION_CENTIPAWN_SCALE;
