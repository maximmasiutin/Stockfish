...
void update_correction_history(const Position& pos,
    ...
    if (depth <= 10 && (ss - 1)->currentMove.is_ok())
    {
        const Square to = (ss - 1)->currentMove.to_sq();
        const Piece  pc = pos.piece_on(to);
        shared.update_contcorr(*(ss - 2)->continuationCorrectionHistory, pc, to, bonus * 129 / 128);
        shared.update_contcorr(*(ss - 4)->continuationCorrectionHistory, pc, to, bonus * 61 / 128);
    }
}
