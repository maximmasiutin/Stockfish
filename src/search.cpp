...
size_t contcorr_hash_2ply(unsigned idx2, unsigned idx3) {
    // Best 2-ply hash uniformity (std=0.413) over odd primes in [257,1535]
    // Do not SPSA-tune: hash multipliers have no smooth optimum.
    constexpr unsigned primeHashMult2Ply = 509;
    return size_t(idx2 * primeHashMult2Ply + idx3);
}
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int jcv =
      ss->ply >= 3 && (ss - 2)->currentMove.is_ok() && (ss - 3)->currentMove.is_ok()
        ? int(shared.joint_correction_entry(
            pos.pawn_key() ^ contcorr_hash_2ply((ss - 2)->contcorrIndex, (ss - 3)->contcorrIndex)))
        : 0;
...
void update_correction_history(const Position& pos,
    ...
    if (ss->ply >= 3 && (ss - 2)->currentMove.is_ok() && (ss - 3)->currentMove.is_ok())
    {
