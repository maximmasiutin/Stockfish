...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int jcv =
      ss->ply >= 2 && (ss - 2)->currentMove.is_ok()
        ? int(shared.joint_correction_entry(pos.pawn_key() ^ size_t((ss - 2)->contcorrIndex)))
        : 0;
...
void update_correction_history(const Position& pos,
    ...
    if (ss->ply >= 2 && (ss - 2)->currentMove.is_ok())
    {
        shared.joint_correction_entry(pos.pawn_key() ^ size_t((ss - 2)->contcorrIndex)) << bonus;
    }
}
