...
// NMP depth-lookup: SPSA-tunable reductions for depths 6-7
int nmpR6 = 9;
int nmpR7 = 9;
TUNE(SetRange(3, 18), nmpR6, nmpR7, SetDefaultRange);
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && !is_loss(beta))
    {
        ...
        // Null move dynamic reduction based on depth (d6-26 lookup, d6-7 SPSA-tunable)
        Depth R;
        {
            constexpr int               offs = 6;
            const std::array<Depth, 21> rv   = {nmpR6, nmpR7, 9,  11, 10, 10, 11, 11, 11, 12, 12,
                                                12,    14,    14, 14, 13, 14, 15, 12, 14, 13};
            R = (depth >= offs && depth < offs + int(rv.size())) ? rv[size_t(depth - offs)]
                                                                 : 7 + depth / 3;
        }
