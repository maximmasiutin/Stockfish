...
Search::Worker::Worker(SharedState&                    sharedState,
    ...
    pawnHistory(
      make_unique_large_page<StatsEntry<std::int16_t, PAWN_HISTORY_LIMIT>[][PIECE_NB][SQUARE_NB]>(
        PAWN_HISTORY_SIZE)),
...
void Search::Worker::clear() {
    ...
    for (size_t i = 0; i < PAWN_HISTORY_SIZE; ++i)
        for (size_t j = 0; j < PIECE_NB; ++j)
            for (size_t k = 0; k < SQUARE_NB; ++k)
                pawnHistory.get()[i][j][k] = -1238;
  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        if (!ttHit && type_of(pos.piece_on(prevSq)) != PAWN
            ...
            pawnHistory.get()[pos.pawn_key() & (PAWN_HISTORY_SIZE - 1)][pos.piece_on(prevSq)][prevSq] << evalDiff * 13;
    }
...
moves_loop:  // When in check, search starts here
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else
            {
                int history = (*contHist[0])[movedPiece][move.to_sq()]
                            ...
                            + pawnHistory.get()[pos.pawn_key() & (PAWN_HISTORY_SIZE - 1)][movedPiece][move.to_sq()];
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        if (type_of(pos.piece_on(prevSq)) != PAWN && ((ss - 1)->currentMove).type_of() != PROMOTION)
            pawnHistory.get()[pos.pawn_key() & (PAWN_HISTORY_SIZE - 1)][pos.piece_on(prevSq)][prevSq]
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    MovePicker mp(pos, ttData.move, DEPTH_QS, &mainHistory, &lowPlyHistory, &captureHistory,
                  contHist, pawnHistory.get(), ss->ply);
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    workerThread.pawnHistory
      .get()[pos.pawn_key() & (PAWN_HISTORY_SIZE - 1)][pos.moved_piece(move)][move.to_sq()]
