...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            {
                int history = int((*contHist[0])[movedPiece][move.to_sq()]) * CONTHIST_SCALE
                            + int((*contHist[1])[movedPiece][move.to_sq()]) * CONTHIST_SCALE
        ...
        if (capture)
            ...
            ss->statScore = 2 * mainHistory[us][move.raw()]
                          + int((*contHist[0])[movedPiece][move.to_sq()]) * CONTHIST_SCALE
                          + int((*contHist[1])[movedPiece][move.to_sq()]) * CONTHIST_SCALE;
