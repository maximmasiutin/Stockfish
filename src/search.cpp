...
HashedContCorrSlot::Tag contcorr_tag_from(uint64_t h) {
    ...
    return t == HashedContCorrSlot::Tag(0) ? HashedContCorrSlot::Tag(1) : t;
}
...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    if (m.is_ok())
    {
        auto&        table = const_cast<HashedContCorrHistory&>(w.hashedContCorrHistory);
        ...
        const HashedContCorrAccess accA{&table[contcorr_idx_from(hashA)], contcorr_tag_from(hashA)};
        const HashedContCorrAccess accB{&table[contcorr_idx_from(hashB)], contcorr_tag_from(hashB)};
