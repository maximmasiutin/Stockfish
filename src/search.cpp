...
uint32_t contcorr_hash_3ply(unsigned idxA, unsigned idxB, unsigned idxC) {
    // FFT enumeration over odd primes in [257, 6140] against the 769-
    // element Piece-based index domain for N=16384: inner P=3527
    // (top-1 at 2-ply), outer P=2803 (top-1 at 3-ply conditional on
    // inner=3527, std=1.041, chi2/mean=0.271).
    // Don't SPSA-tune: hash multipliers have no smooth optimum.
    constexpr uint32_t primeHashMult2Ply = 3527;
    constexpr uint32_t primeHashMult3Ply = 2803;
