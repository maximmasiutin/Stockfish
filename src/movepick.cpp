...
struct SimdSorter {
    ...
    void write_sorted(ExtMove* dst, int count) const {
        ...
        // Build a 32-bit lane mask with the low `count` bits set; BZHI(~0u, 32)
        // returns ~0u per the Intel ISA (index >= operand width keeps source).
        const uint32_t bits = _bzhi_u32(~0u, unsigned(count));

        _mm512_mask_storeu_epi64(dst, __mmask8(bits), v0);
        _mm512_mask_storeu_epi64(dst + 8, __mmask8(bits >> 8), v1);
        _mm512_mask_storeu_epi64(dst + 16, __mmask8(bits >> 16), v2);
        _mm512_mask_storeu_epi64(dst + 24, __mmask8(bits >> 24), v3);
    }
};
