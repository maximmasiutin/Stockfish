...
ExtMove* MovePicker::score(const MoveList<Type>& ml) {
    ...
    for (auto move : ml)
    {
        ...
        {
            const auto& mainHistoryEntry = (*mainHistory)[us][m];
            const auto& lowPlyHistoryEntry =
              (*lowPlyHistory)[std::min(ply, LOW_PLY_HISTORY_SIZE - 1)][m];
            ...
            if (ply < LOW_PLY_HISTORY_SIZE)
                m.value += 8 * lowPlyHistoryEntry / (1 + ply);
