...
ExtMove* MovePicker::score(const MoveList<Type>& ml) {
    ...
    for (auto move : ml)
    {
        ...
        {
            // histories
            m.value = 2 * (*mainHistory)[us][history_slot(m)];
            ...
            if (ply < LOW_PLY_HISTORY_SIZE)
                m.value += 8 * (*lowPlyHistory)[ply][history_slot(m)] / (1 + ply);
        }
        ...
        else  // Type == EVASIONS
        {
            if (pos.capture_stage(m))
                ...
                m.value = (*mainHistory)[us][history_slot(m)] + (*continuationHistory[0])[pc][to];
        }
    }
    return it;
}
