...
sf_noinline std::size_t main_hist_compact_index_special(std::uint32_t r) {
    ...
    // Order by typical VLTC frequency (120+1.2, 1T, initial position, no
    // adjudication, 2 games, 16,521,245 non-NORMAL calls sampled across
    // both engine processes):
    //   CASTLING   14,462,482  87.54% of non-NORMAL calls
    //   PROMOTION   2,058,763  12.46%
    //   EN_PASSANT          0   0.00% (rare; never observed in this sample)
    if (type == 3u)
        // CASTLING
