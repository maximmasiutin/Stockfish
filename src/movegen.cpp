...
// AoS interleaved layout: each (tag, addr) pair sits in 4 contiguous bytes,
// so a hit reads a single 32-bit word and stays within one cache line.
struct HotsetEntry {
    std::uint16_t tag;
    std::uint16_t addr;
};
static_assert(sizeof(HotsetEntry) == 4, "HotsetEntry must be 4 bytes for the AoS hot-set layout");

struct alignas(128) HotsetTable {
    std::array<HotsetEntry, 32> entries;
};
static_assert(sizeof(HotsetTable) == 128, "HotsetTable must be exactly 128 bytes");
static_assert(alignof(HotsetTable) == 128, "HotsetTable must be aligned to 128 bytes");

// 128 bytes total, aligned to 128 so the entire payload sits in two
// consecutive 64-byte cache lines: entries 0-15 in line 0, 16-31 in line 1.
// A hit touches only the line containing its idx.
...
sf_noinline std::size_t low_ply_freq_index(Move m) {
    ...
    // Hot-set lookup: greedy-filled top-frequency raws. On miss, fall through.
    {
        ...
        const HotsetEntry   e   = hotset.entries[idx];
        if (e.tag == r)
            return e.addr;
    }
