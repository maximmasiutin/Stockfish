...
// Cold tail of low_ply_freq_index. Fully branchless: the PROMOTION /
// CASTLING dispatch is a SUB+SAR mask blend, and the castling side bit
// uses an arithmetic-shift sign extraction. EN_PASSANT (type == 2)
// hits the castling path, matching the v2 layout.
sf_noinline std::size_t low_ply_freq_index_special(std::uint32_t r) {
    ...
    // half = from >> 5: 0 for ranks 1-4, 1 for ranks 5-8 (not the WHITE/BLACK enum).
    ...
    // The mask blend below routes type=2 to cast_slot silently if asserts are off.
    assert(type == 1u || type == 3u);
    assert(promo < 3u);

    const std::uint32_t promo_slot = 4320u + (half << 5) + file * 3u + promo;
